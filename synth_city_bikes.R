...
read_bike_lanes<- function(folder_name, 
  ...
  location <- paste("city_bike_lanes/01_easy_to_clean/", folder_name, sep = "")
...
charlette_1 <- read_bike_lanes("charlette_1") %>%
                mutate(type = "shared_lane")

charlette_2 <- read_bike_lanes("charlette_2", column_with_type = "Bike_Lanes", 
...
#dc_1 
dc_1 <- st_read("city_bike_lanes/02_hard_to_clean/dc_1/") %>%
...
#Kansas City
kansas_city <- st_read("city_bike_lanes/02_hard_to_clean/kansas_city/a00000009.gdbtable") %>%
  st_zm(drop = TRUE) %>%
  st_transform(2163) %>%
  mutate(type = case_when(
...
memphis <- st_read("city_bike_lanes/02_hard_to_clean/memphis/v105/bike_ped__facility_data.gdb") %>%
  st_zm(drop = TRUE) %>%
  st_transform(2163) %>%
  mutate(type = case_when(
...
#Minneapolis
minneapolis <- st_read("city_bike_lanes/02_hard_to_clean/minneapolis") %>%
  st_zm(drop = TRUE) %>%
  st_transform(2163) %>%
...
#nashville lanes:
nashville_1 <-st_read("city_bike_lanes/02_hard_to_clean/nashville/BikewayExport011121.shp") %>%
  st_transform(2163) %>%
  st_zm(drop = TRUE) %>%
  mutate(type = case_when(
...
#nashville trails:
nashville_2 <-st_read('city_bike_lanes/02_hard_to_clean/nashville/ExistingTrails_Export011121.shp') %>%
  st_transform(2163) %>%
  st_zm(drop = TRUE) %>%
...
#New York
new_york <- st_read(dsn = "city_bike_lanes/02_hard_to_clean/new_york/a00000009.gdbtable") %>%
  st_transform(2163) %>%
  filter(BIKING == "Y") %>%
  mutate(type = case_when(
...
philadelphia_2 <- st_read("city_bike_lanes/02_hard_to_clean/philadelphia_2/PhiladelphiaBikeConnectorStreets201204.shp") %>%
  st_transform(2163) %>%
  mutate(type = case_when(
...
philadelphia_3 <- st_read("city_bike_lanes/02_hard_to_clean/philadelphia_2/PhiladelphiaBikeRegionalConnections201204.shp") %>%
  st_transform(2163) %>%
  mutate(type = case_when(
...
philadelphia_4 <- st_read("city_bike_lanes/02_hard_to_clean/philadelphia_2/PhiladelphiaBikeTrailsSidepaths201209.shp") %>%
  st_transform(2163) %>%
  select() %>%
  mutate(type = "separated_path") %>%
...
#Pittsburgh Lanes:
pittsburgh_1 <- st_read("city_bike_lanes/02_hard_to_clean/pittsburgh/Bike\ Lanes.shp") %>%
  st_transform(2163) %>%
  select() %>%
  mutate(type = "painted_lane")%>%
...
pittsburgh_2 <- st_read("city_bike_lanes/02_hard_to_clean/pittsburgh/Cautionary Bike Route.shp") %>%
  st_transform(2163) %>%
  select() %>%
  mutate(type = "shared_lane")%>%
...
pittsburgh_3 <- st_read("city_bike_lanes/02_hard_to_clean/pittsburgh/On Street Bike Route.shp") %>%
  st_transform(2163) %>%
  select() %>%
  mutate(type = "shared_lane")%>%
...
#Protected Bike Lane:
pittsburgh_4 <- st_read("city_bike_lanes/02_hard_to_clean/pittsburgh/Protected Bike Lane.shp") %>%
  st_transform(2163) %>%
  select() %>%
...
#Sharrow:
pittsburgh_5 <- st_read("city_bike_lanes/02_hard_to_clean/pittsburgh/Sharrows.shp") %>%
  st_transform(2163) %>%
  select() %>%
  mutate(type = "shared_lane")%>%
...
#Path
pittsburgh_6 <- st_read("city_bike_lanes/02_hard_to_clean/pittsburgh/trails.shp") %>%
  st_transform(2163) %>%
  select() %>%
  mutate(type = "separated_path")%>%
...
#Providence:
providence <- st_read("city_bike_lanes/02_hard_to_clean/providence") %>%
  st_transform(2163) %>%
...
salt_lake_1 <- st_read("city_bike_lanes/02_hard_to_clean/salt_lake_city_1")
...
salt_lake_2 <- st_read("city_bike_lanes/02_hard_to_clean/salt_lake_city_2") %>%
  st_transform(2163) %>%
  filter(Class == "Path") %>%
  filter(Status == "EXISTING") %>%
...
#Tucson:
tucson <- st_read("city_bike_lanes/02_hard_to_clean/tucson") %>%
  st_transform(2163) %>%
  filter(BIKERTE_DS != "Planned Bike Route with Striped Shoulder" &
         BIKERTE_DS != "Planned Shared-use Path" &
         BIKERTE_DS != "Hiking/Mountain Bike/Equestrian/Wildland Trail" &
         BIKERTE_DS != "Residential Streets") %>%
  mutate(type = case_when(
    BIKERTE_DS %in% c("Bike Route with Striped Shoulder", 
                      "Key Connecting Streets",
                      "Bus/Bike Lane",
                      "Shared Lane Markings")  ~"shared_lane",
    BIKERTE_DS %in% c("Bike Route")  ~"painted_lane",
    BIKERTE_DS %in% c("")  ~"protected_lane",
    BIKERTE_DS %in% c("Shared-use Path")  ~"separated_path",
    BIKERTE_DS %in% c("Bike Blvd/Shared Lane Markings")  ~"bvld")) %>%
  select(type) %>%
  mutate(city = "tucson")
  
  
#Combine all the city dataframes ======
city_bikes <- rbind(atlanta, baltimore, boston, buffalo, charlette_1, 
                    charlette_2, chicago, cleveland, columbus, dallas_1, dallas_2,
                    dc_1, dc_2, denver, detroit_1, detroit_2, ft_worth, 
                    houston, indianapolis_1, indianapolis_2, 
                    jacksonville_1, jacksonville_2, las_vegas, 
                    los_angeles, louisville, miami_1, miami_2, 
                    minneapolis, nashville_1, nashville_2,new_orleans, 
                    new_york, orlando,philadelphia_1, philadelphia_2, philadelphia_3, 
                    ...
                    tucson, virginia_beach)
city_bikes <- city_bikes %>%
  mutate(type = ifelse(is.na(type), "unknown", type)) 


st_write(city_bikes, "city_bike_lanes/city_bikes.geojson")
